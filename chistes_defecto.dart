//..lib/data/chistes_defecto.dart
final Map<String, List<String>> chistesPorDefecto = {
  'Animales': [
    '¿Qué hace una abeja en el gimnasio?\n¡Zum-ba!',
    'Dos patos hacen una carrera.¿Cómo acaban?\n¡EMPATADOS!',
    '¿Qué hace un perro con un taladro?\nTa-ladrando.',
    '¿Cuál es el animal que es dos en uno?\nEl gato, porque es gato y araña.',
    '¿Por qué las vacas viajan a Nueva York?\nPara ver los muuuusicales.',
 
  ],
  'Tecnología': [
    '¿Qué le dice una impresora a otra?\n¿Esa hoja es tuya o es una impresión mía?',
    '¿Por qué los programadores confunden Halloween y Navidad?\nPorque OCT 31 = DEC 25.',
    'Mi mujer me dijo que necesitaba más espacio.\n¿Y qué hiciste?\nLa regalé un disco duro de 2Tb.',
    '- Mamá ¿qué haces enfrente del ordenador con los ojos cerrados?\n- Nada hijo, que Windows me dijo que cerrara las pestañas...',
    '¿Cuál es la mayor mentira de todo el universo?\n"He leído y acepto los Términos y Condiciones".',
  ],
  'Escuela': [
    '¿Cómo se dice pañuelo en japonés?\nSaka-moko.',
    '¿Por qué estudias en el suelo?\nPorque quiero estar por debajo del promedio.',
    'Papá, hoy no puedo ir a la escuela.\n¿Por qué no hijo?\nNo me siento bien.\n¿Por qué no te sientes bien?\nPorque tengo que ir a la escuela.',
    'Mamá, en la escuela me dicen que soy un interesado.\n¿Quién te lo dijo hijo?\n¡Si me das cinco dólares te lo digo!',
    'Mamá, mamá, en el colegio me dicen dientudo.\nBueno hijo pero cierra la boca que rayas el piso.',
  ],
  'Parejas': [
    'Mi esposa y yo fuimos felices durante 20 años…\nluego nos conocimos.',
    '¿Qué hace una pareja en el gimnasio?\nPesan su relación.',
  ],
};
